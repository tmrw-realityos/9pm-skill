...
- **`isOwner` identifies the app's owner** — `true` when the signed-in user is the person who deployed the app, with no configuration. When the user asks for an owner-only or admin area, gate those routes on `ctx.user.isOwner` **server-side** (never in client code, never on a client-supplied value). It marks exactly one person; additional admins or roles are the app's own data.
...
- Invites reach people two ways. By default the invited person signs in and finds the group waiting (add-by-email, or a shareable link). If the app is deployed with `--with-email`, a signed-in user can also send a **managed invitation email** — `POST /__9pm/email/invite` with `{ to, mode: "invited" | "added", inviterName, thingName }` from the app's own UI. 9pm sends it, rate-limits it, and handles opt-outs; the app never touches an email provider. Requires managed accounts, works on public apps only, and is budgeted (roughly a couple dozen invites per person per day). Names can't contain links. Do not build your own email sending for invites.
  - **`202` means accepted, not delivered.** By design the endpoint returns the same `202` whether the email was sent, silently dropped because the recipient opted out, or failed at the provider — this is deliberate (opt-outs and delivery must not be probeable). A `429 rate_limited` means the budget/cooldown was hit. So: never tell the user "email sent" — say "invite sent if that address accepts email", and **always keep a shareable link available** (both after a `429` *and* after a `202`), since a `202` is not proof anything arrived.
